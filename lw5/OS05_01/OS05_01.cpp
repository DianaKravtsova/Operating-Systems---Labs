...
void printProcessPrty(HANDLE h) {
	...
	std::cout << NORMAL_PRIORITY_CLASS << ' ' << BELOW_NORMAL_PRIORITY_CLASS << ' ' << HIGH_PRIORITY_CLASS << std::endl;
	switch (prty) {
...
void printThreadPrty(HANDLE h) {
	DWORD prty = GetPriorityClass(h);
	std::cout << THREAD_PRIORITY_LOWEST << ' ' << THREAD_PRIORITY_NORMAL << ' ' << THREAD_PRIORITY_HIGHEST << std::endl;
